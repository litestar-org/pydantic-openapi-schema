...
A `Reference Object <https://spec.openapis.org/oas/v3.1.0#referenceObject>`__ can link to a response
that the `OpenAPI Object's components/responses <https://spec.openapis.org/oas/v3.1.0#componentsResponses>`__ section defines.

Patterned Fields
{httpStatusCode}: Optional[Union[Response, Reference]]

Any `HTTP status code <https://spec.openapis.org/oas/v3.1.0#httpCodes>`__ can be used as the property name,
but only one property per code, to describe the expected response for that HTTP status code.

A `Reference Object <https://spec.openapis.org/oas/v3.1.0#referenceObject>`__ can link to a response
that is defined in the `OpenAPI Object's components/responses <https://spec.openapis.org/oas/v3.1.0#componentsResponses>`__ section.
